...
m_time(30)
{
	...
	for (int i = 0; i < 300; i++){
		m_obj_list.emplace_back(std::make_shared<leaf>(app_env));
	}
	...
	for (auto& obj : m_obj_list){
		if (obj->GetType() == Object::Type::HolyGhost){
			std::dynamic_pointer_cast<CHolyGhost>(obj)->Start(m_obj_list);
		}
	}
